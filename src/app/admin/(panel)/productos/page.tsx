      ...
      <div className="border border-kult-border overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-kult-border">
              {["Ref", "Nombre", "Marca", "Cat.", "Precio", "Stock", "Activo", ""].map((h) => (
          ...
          <tbody>
            {products.map((p: {
              ...
              return (
                <tr key={p.id} className="border-b border-kult-border hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <span className={totalStock < 5 ? "text-kult-fire font-bold" : ""}>{totalStock}</span>
                  </td>
                  <td className="px-5 py-3">
                    <span className={`text-xs font-bold ${p.is_active ? "text-green-400" : "text-kult-muted"}`}>
                      {p.is_active ? "✓" : "—"}
                    </span>
                  </td>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      <Link href={`/admin/productos/${p.id}`} className="text-xs text-kult-neon hover:underline">Editar</Link>
        ...
        {products.length === 0 && (
          <div className="text-center py-16">
            <p className="font-display text-3xl tracking-widest opacity-30">SIN PRODUCTOS</p>
            <Link href="/admin/productos/nuevo" className="mt-4 inline-block text-kult-neon underline text-sm">
              Crear el primero
            </Link>
          </div>
        )}
