...
export default async function PedidosPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="p-8 space-y-6">
      <h1 className="font-display text-4xl tracking-widest">PEDIDOS</h1>
          ...
          <a
            ...
            className={`px-3 py-1.5 text-xs uppercase tracking-widest border transition-colors ${
              searchParams.status === s || (!searchParams.status && !s)
                ...
                : "border-kult-border hover:border-kult-muted"
            }`}
          ...
          <tbody>
            {orders.map((order: {
              ...
              <tr key={order.id} className="border-b border-kult-border hover:bg-white/5 transition-colors">
                ...
                <td className="px-5 py-3">
                  <a href={`/admin/pedidos/${order.id}`} className="text-xs text-kult-neon hover:underline">Ver</a>
                </td>
              </tr>
            ))}
          </tbody>
